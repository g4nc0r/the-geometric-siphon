...
% Author-date citations (Harvard style, finance/economics convention)
...
\vspace{4pt}
{\small\textbf{JEL classification:} G12, G14, G19, G23}
\end{center}
...
\begin{abstract}
This paper identifies and formalises the \textbf{Geometric Siphon}, a previously undocumented mechanism in concentrated liquidity (CL) portfolio management. When multiple CL positions share a depositor-level token balance and are autonomously rebalanced, the token-ratio mismatch between old and new tick ranges produces residuals that flow through the shared balance, creating capital transfers between positions in independent pools.

I derive a closed-form expression for the geometric residual $\Delta R$ from the Uniswap V3 amount equations and prove that $\Delta R = 0$ if and only if the old and new ranges demand the same token ratio at the current price (Theorem~\ref{thm:residual}). The theorem is verified with a Foundry test suite running against unmodified Uniswap V3 contracts on Base mainnet: the residual is measurable on range changes (2{,}061 USDC in the reference scenario), below $10^{-6}$ USD on same-range rebalances, and scales linearly with position size ($4\times$ position $\to$ $4\times$ residual).

Empirical analysis of 1{,}380 rebalance events across 39 positions, five depositor groups, two contract versions and seven days on Aerodrome (Base) reveals a two-mechanism structure: \textbf{Stage~1} (geometric creation, ${\sim}5\%$ of events) produces residuals via range changes that feed a shared dust pool; \textbf{Stage~2} (dust redistribution, ${\sim}95\%$ of events) redistributes accumulated dust through same-range rebalances, governed by the position-to-pool liquidity ratio. The decomposition of observed net flow $D$ into gross residual $\Delta R$ and swap slippage $S$ holds at Spearman $\rho(\Delta R, D) = 0.976$, with median swap price impact $0.35\%$.

I prove that single-pool positions sharing a contract balance converge to equal value (Theorem~\ref{thm:convergence}). A controlled sweep confirms this empirically: three positions deployed at size ratio $1{:}3{:}9$ collapsed to $1.37{:}1{:}1.20$ within 33 events, with the original size ordering reversed. Multi-pool portfolios settle at position-specific equilibria determined by pool depth. Token-level decomposition across ten pools and five depositor groups identifies the \textbf{Connector Rule}: the per-pool flow driver is the token shared with more portfolio positions, and the same pool exhibits different driver tokens under different portfolio topologies.

I prove that a position rebalancing repeatedly without swap correction ($S = 0$) decays geometrically: $V_K \leq V_0(1 - \alpha_{\min})^K$ where $\alpha_{\min}$ is the minimum residual fraction across $K$ consecutive donation events (Theorem~\ref{thm:extinction}). The bound follows from the CL amount equations, the $\min$ constraint in the liquidity-from-amounts function, and the finiteness of the shared dust pool. The theorem is verified with four additional Foundry tests and validated against the complete lifecycle of a USDC/ZARP position that lost $99.8\%$ of its capital ($\text{\$942} \to \text{\$1.59}$) in 14 zero-swap rebalances over 65 hours; the extinction bound predicted 11 rebalances at the observed average leak rate, against 10 donation events observed.
\end{abstract}
...
Concentrated liquidity, introduced by Uniswap V3 \citep{adams2021}, requires active management. Every major CL automation platform (Gamma Strategies, Arrakis Finance, Beefy CLM) uses a \textbf{vault-per-pool architecture} in which one vault manages one token pair and leftover tokens from rebalances are reinvested into the same position.
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Closed-form geometric residual.} $\Delta R/V$ is derived as a function of old range, new range and current price from the Uniswap V3 amount equations (\S\ref{sec:residual}). The residual is zero if and only if the old and new ranges demand the same token ratio at current price (Theorem~\ref{thm:residual}).
    ...
    \item \textbf{Equilibrium proof.} Single-pool positions converge to equal value under repeated rebalancing (Theorem~\ref{thm:convergence}, \S\ref{sec:equilibrium}), confirmed empirically with a controlled $V/\mathcal{L}$ sweep experiment (\S\ref{sec:vl-sweep}).
    ...
    \item \textbf{Simpson's paradox in CL analysis.} $V/\mathcal{L}$ and volatility $\sigma$ are complementary predictors that reverse sign when data are aggregated (\S\ref{sec:predictive}).

    \item \textbf{Zero-swap extinction.} Positions that rebalance without swaps decay geometrically (Theorem~\ref{thm:extinction}, \S\ref{sec:extinction}). A USDC/ZARP position provides a complete empirical lifecycle: $\text{\$942} \to \text{\$1.59}$ in 14 zero-swap rebalances (\S\ref{sec:zarp}). Four additional Foundry tests confirm the theorem (\S\ref{sec:foundry}, 7/7 total).

    \item \textbf{Companion paper.} Three directional properties of the residual (monotonicity in displacement, directional asymmetry in USD terms, and exit-side asymmetry) are derived and verified as Theorems~4--6 in \citet{ryan2026b}, sharing the same Foundry verification suite.
\end{enumerate}
...
\citet{adams2021} introduced concentrated liquidity in Uniswap V3. \citet{fan2023} formulated the strategic LP problem of choosing tick ranges to maximise fees net of repositioning costs, treating each position in isolation. This paper studies what happens when positions share depositor-level balances.

\citet{heimbach2022} model and empirically analyse V3 LP risks and returns, finding wide cross-strategy variation that rewards active management. This paper isolates the residual from fee accrual and impermanent loss by taking the rebalancing decision as given.

\citet{hasbrouck2025} model V3 CL provision as a covered call sold at intrinsic rather than market value, so LPs forgo the option's time premium in exchange for fees, and equilibrium liquidity provision decreases in that time premium. The geometric residual is a separate loss arising at the rebalance itself, determined by the geometry of $R(s, s_a, s_b)$ rather than by option-theoretic exposure between rebalances.

\citet{milionis2022b} introduced LVR (Loss-Versus-Rebalancing), the return gap between an AMM LP and a continuously rebalancing benchmark, driven by arbitrageurs trading against stale AMM prices. \citet{milionis2024} extend the framework with trading fees and discrete block timing. LVR is a continuous holding cost; the geometric residual is a discrete event cost at rebalance, determined by the V3 amount equations rather than by arbitrage. The decomposition in \S\ref{sec:decomposition} attributes over $97\%$ of observed flow to the residual rather than to swap-side friction.

\citet{cartea2024} introduce \emph{predictable loss}, a closed-form measure of the unhedgeable losses an LP incurs from holding assets in a CL pool, comprising convexity loss from informed trading and opportunity cost on locked capital. Predictable loss accrues between rebalances; the geometric residual arises at the rebalance itself, depends on range geometry rather than price trajectory, and is zero on same-range rebalances regardless of price travel (Theorem~\ref{thm:residual}).

\citet{willetts2026} measure arbitrage efficiency and rebalancing costs in dynamic weight AMM pools on Ethereum and Base using LVR benchmarks, the closest empirical context for per-rebalance costs on Base L2.

\textbf{Balancer} \citep{martinelli2019} rebalances via its AMM invariant within a single pool, \textbf{Set Protocol} uses oracle-triggered swaps, and \textbf{Yearn} deploys capital across strategist-managed strategies. All require a shared pool invariant, external price feeds, or manual intervention; the siphon arises from CL geometry alone.

\textbf{Maverick Protocol} \citep{baxley2023} introduced directional liquidity modes where bins follow price automatically. Its `Mode Both' is the closest analogue to siphon-driven repositioning, being continuous, automatic, and oracle-free. Maverick operates within a single position; the siphon operates \emph{across} positions in different pools.
...
Flow notation. $\mathcal{L}$ denotes the total liquidity of the pool containing the position, distinguished from the position's own $L$. For a rebalance, $\Delta R$ denotes the gross geometric residual predicted by~\eqref{eq:amounts} under an isolated withdraw-and-mint sequence (characterised in Theorem~\ref{thm:residual} below); $S$ denotes the swap slippage absorbed by an internal rebalance swap; and $D = \Delta R - S$ denotes the observed net dust flow. Theorem~\ref{thm:residual} characterises $\Delta R$; \S\ref{sec:results} reports $D$ and $S$ empirically.
...
The new range demands ratio $R_{\text{new}} = R(s, s_{a'}, s_{b'})$. If $R_{\text{old}} \neq R_{\text{new}}$, the withdrawn tokens cannot be fully deposited: one is the \textbf{binding constraint}, the other has a surplus. The new liquidity is the minimum of the per-token capacities,
...
\begin{enumerate}[label=(\roman*),leftmargin=*]
    \item \textbf{Width change.} Widening ($w_{\text{new}} > w$) gives $R_{\text{new}} < R_{\text{old}}$ (position produces token0 excess); narrowing gives $R_{\text{new}} > R_{\text{old}}$ (token1 excess). In a shared-balance context the widening case tends towards net absorption ($D/V > 0$), narrowing towards net donation ($D/V < 0$).
    \item \textbf{Displacement.} For a rebalance recentred on the current price after the tick moves by $\delta$ from the old range midpoint, $|\Delta R/V|$ is strictly increasing in $|\delta|$ for $|\delta| \leq w/2$. A detailed proof is given as Theorem~4 in \citet{ryan2026b}.
\end{enumerate}
\end{corollary}
...
\textbf{Stage~1, Geometric Creation (the source).} Range-change rebalances (${\sim}5\%$ of events) produce geometric residuals via equation~\eqref{eq:cases}. The position manager's internal swap absorbs ${\sim}94\%$ of the token ratio mismatch; the remaining ${\sim}6\%$ enters the shared dust pool as unmatched tokens. Without range changes, the dust pool drains to zero.
...
$V/\mathcal{L}$ governs Stage~2. Low-$V/\mathcal{L}$ positions withdraw small amounts relative to the dust pool, so contract dust represents a large proportional gain. High-$V/\mathcal{L}$ positions withdraw large amounts, so the dust pool is a rounding error relative to their size. The relationship is non-linear. Absorption peaks at $V/\mathcal{L} \approx 0.05$--$0.10$, crosses zero near $V/\mathcal{L}^* \approx 0.13$, and donation intensifies monotonically above that threshold (\S\ref{sec:vl-sweep}). Most events ($73\%$) cluster at $V/\mathcal{L} < 0.01$ where effects are negligible, making the aggregate linear correlation weak ($r \approx -0.1$) despite a clear directional relationship across the full $V/\mathcal{L}$ range.
...
\end{equation}
Different pools have different $L_{\text{pool}}$, so equal $V/\mathcal{L}$ means different equilibrium sizes. Positions in deeper pools settle at larger values, and hub-spoke topologies are stable because the deep hub sustains a large position whilst volatile spokes sustain smaller ones.
...
Under sustained $S = 0$ conditions with stable or declining pool liquidity $\mathcal{L}$, the value sequence $V_0, V_1, \ldots$ is monotonically decreasing. Recovery requires a change in $\mathcal{L}$ sufficient to reduce $V/\mathcal{L}$ below the absorption crossover ($V/\mathcal{L}^* \approx 0.13$, \S\ref{sec:predictive}).
\end{corollary}
...
\midrule
A & 3 & Similar $V/\mathcal{L}$, different volatility & $\sigma$ isolation \\
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{$V/\mathcal{L}$ sweep experiment.} Three positions at $1{:}3{:}9$ size ratio in the same pool, isolating position size as the sole variable.
    \item \textbf{Fiat FX weekend experiment.} Five stablecoin FX pairs deployed on a Sunday (markets closed), providing $\sigma \approx 0$ conditions that isolate the $V/\mathcal{L}$~effect from volatility.
\end{itemize}
...
\begin{table}[H]
\centering\small
\begin{tabular}{@{}lp{6.4cm}lp{4cm}@{}}
\toprule
ID & Hypothesis & Source & Result \\
\midrule
H1 & Flow is geometric, not frictional       & Phase 1            & Yes, $\rho(\Delta R, D) = 0.976$ \\
H2 & $V/\mathcal{L}$ and $\sigma$ are complementary    & Phase 1            & Yes, Simpson's paradox confirmed \\
H3 & Same-pool positions converge            & Theory (Thm~\ref{thm:convergence}) & Yes, $1{:}3{:}9 \to 1.37{:}1{:}1.20$ \\
H4 & Multi-pool positions settle at equal $V/\mathcal{L}$ & Theory~\eqref{eq:multipool} & Yes, hub-spoke stable \\
...
H8 & $\sigma = 0$ isolates $V/\mathcal{L}$ as sole predictor & Phase 2 design & Yes, fiat FX confirms \\
\bottomrule
\end{tabular}
\end{table}
...
Pooling all 545 events (with $|D| > \$0.50$) gives $\rho = 0.329$ for $V/\mathcal{L}$ and $\rho = -0.367$ for $\sigma$, suggesting volatility \emph{reduces} flow size. This is a Simpson's paradox.
...
\begin{tabular}{@{}lrrr@{}}
\toprule
Group & $N$ & $V/\mathcal{L} \to |D|$ & $\sigma_{1h} \to |D|$ \\
\midrule
...
$V/\mathcal{L}$ and $\sigma$ are complementary: each dominates exactly when the other lacks cross-position variance. B mixes CL1/CL10/CL100 ($25\times$ $V/\mathcal{L}$ spread), so $V/\mathcal{L}$ captures the signal. A has near-identical $V/\mathcal{L}$ across positions, so $\sigma$ takes over.
...
\subsubsection*{Nonlinear $V/\mathcal{L}$ transfer function (Phase 2, $N = 606$)}

The relationship between $V/\mathcal{L}$ and $D/V$ is non-linear. Bucketed analysis across all Phase 2 events with $V/\mathcal{L}$ data:

\begin{table}[H]
\centering
\begin{tabular}{@{}lrrl@{}}
\toprule
$V/\mathcal{L}$ range & $N$ & Mean $D/V$ & Behaviour \\
\midrule
...
\caption{$V/\mathcal{L}$ transfer function: absorption peaks at $V/\mathcal{L} \approx 0.05$--$0.10$, crosses zero near $V/\mathcal{L}^* \approx 0.13$, and donation intensifies above. Crypto pools (circles) and stablecoin FX pools (squares) shown; marker size proportional to $N$.}
\label{fig:vl-transfer}
\end{figure}

The non-monotonic shape follows from the two-mechanism structure. At very low $V/\mathcal{L}$, positions are too small for contract dust to materially affect them; at moderate $V/\mathcal{L}$ ($0.05$--$0.10$), dust absorption is proportionally large; above the crossover $V/\mathcal{L}^* \approx 0.13$, positions donate more than they absorb. The $73\%$ of events at $V/\mathcal{L} < 0.01$ make linear regression misleading ($r \approx -0.1$), but the directional relationship is clear across the full range.

$\sigma$ amplifies magnitude ($\rho = +0.26$ for $\sigma_{1h}$ vs $|D/V|$, $N = 536$) but does not determine direction. High $\sigma$ + high $V/\mathcal{L}$ compounds donation; high $\sigma$ + low $V/\mathcal{L}$ compounds absorption.
...
\subsection{Single-pool equilibrium: the $V/\mathcal{L}$ sweep}\label{sec:vl-sweep}
...
\midrule
Small  & \$199 ($V/\mathcal{L} = 0.021$)   & ${\sim}\$900$ & \textbf{$+\$737$} & Absorber \\
Medium & \$597 ($V/\mathcal{L} = 0.063$)   & ${\sim}\$656$ & $+\$84$           & Neutral (flipped) \\
Large  & \$1{,}799 ($V/\mathcal{L} = 0.189$) & ${\sim}\$784$ & \textbf{$-\$933$} & Donor \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
$V/\mathcal{L}$ range & Mean $D/V$ & Behaviour \\
\midrule
...
The crossover $V/\mathcal{L}^* \approx 0.13$--$0.15$ in liquidity ratio terms. CADC/USDC confirms this independently: pool TVL fluctuated around a fixed-size position, causing $V/\mathcal{L}$ to traverse $0.003$ to $0.32$ and the position to role-flip multiple times at the crossover boundary.
...
\textbf{Multi-pool stability.} Unlike single-pool positions (which equalise), multi-pool positions do not converge to equal value. The hub maintains a larger position because WETH/USDC CL50 has deeper TVL (lower $V/\mathcal{L}$ at the same capital). Equation~\eqref{eq:multipool} predicts this: equal $V/\mathcal{L}$ across pools means position sizes proportional to pool depth.
...
Five stablecoin FX pairs deployed on a Sunday (markets closed, $\sigma \approx 0$) provide a natural control experiment isolating $V/\mathcal{L}$ from volatility:

\begin{table}[H]
\centering
\begin{tabular}{@{}lrrrr@{}}
\toprule
Position & $V/\mathcal{L}$ & $\sigma_{24h}$ & Cumulative dust & Swaps \\
\midrule
...
ZARP ($V/\mathcal{L} \approx 0.96$, meaning the position accounts for nearly all the pool's liquidity) donated a mean $27\%$ of its value per donation event. All five FX pairs were deployed with equal capital. ZARP shrank steadily ($V/\mathcal{L}$: $0.958 \to 0.974 \to 0.992$) whilst KRWQ and CADC grew. Capital was conserved at portfolio level.

FX markets were closed and no swaps occurred in any fiat rebalance. Every observed capital flow is therefore attributable to CL geometry alone, confirming $V/\mathcal{L}$ as the sole predictor when $\sigma$ is eliminated.
...
\toprule
Event & Time (UTC) & Pre (\$) & Post (\$) & Dust $D$ & $V/\mathcal{L}$ & $\alpha$ & Pool $\mathcal{L}$ \\
\midrule
...
\textbf{Three phases.} Events 1--4: the position dominated the pool ($V/\mathcal{L} > 0.95$) and donated on every rebalance. Events 5--8: $\mathcal{L}$ fluctuated by $4\times$, producing alternating donation and absorption. Events 9--14: $\mathcal{L}$ increased $20\times$ between events 11 and 12, reducing $V/\mathcal{L}$ to $0.001$. The final rebalance produced $\alpha = 0.99$.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Fork tests} against unmodified Aerodrome Slipstream contracts on Base mainnet via Foundry's \texttt{vm.create\-Select\-Fork}, exercising the same scenarios on real on-chain V3 geometry. These give the canonical demonstration that the residual is a property of unmodified V3 contracts, with the trade-off that the numerical residuals are non-deterministic between runs as the live pool state shifts.
\end{itemize}
...
Same setup, same price movement, but re-mint into the \textbf{identical range}. Residual: \textbf{29~wei}\linebreak ($<\!10^{-6}$ USD). Pure integer rounding from Solidity's floor division.

Assertion: \texttt{assertLt(residual1, 1e3)} . \textbf{PASSED}.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item A $4\times$ position produces a $4\times$ residual (ratio $4.00$), so the $\Delta R/V$ formulation is well-defined and independent of $L$ in the range where $\phi$ is approximately linear.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Single operator, single DEX, single chain.} All data come from one operator's positions on Aerodrome (Base). Theorem~\ref{thm:residual} guarantees the residual exists on all V3-style implementations, but the empirical dynamics (two-mechanism split, $V/\mathcal{L}$ transfer function, equilibrium convergence rate) may differ on other protocols with different fee structures or rebalancing engines.
    ...
    \item \textbf{Equilibrium timescale.} Theorem~\ref{thm:convergence} proves convergence but does not bound the rate. The $V/\mathcal{L}$ sweep shows rapid initial convergence but the long-run steady state is unobserved.
\end{enumerate}
...
The geometric residual follows from the Uniswap V3 amount equations. Any CL range change that alters the required token ratio at current price produces a non-zero token surplus or deficit (Theorem~\ref{thm:residual}), verified against unmodified Slipstream contracts on Base via a Foundry fork test. Empirically, this residual accounts for over $97\%$ of inter-position flow ($\rho = 0.976$), with swap friction contributing $0.35\%$ median price impact. Range changes (${\sim}5\%$ of events) create the residual and feed it into a shared dust pool. Same-range rebalances (${\sim}95\%$) redistribute it, governed by $V/\mathcal{L}$. Single-pool portfolios converge towards equal value (Theorem~\ref{thm:convergence}). Multi-pool portfolios form stable hub-spoke topologies where token connectivity determines the flow direction.

When the swap mechanism is absent ($S = 0$), the full geometric residual leaks on every rebalance and positions decay geometrically (Theorem~\ref{thm:extinction}). A USDC/ZARP position lost $99.8\%$ of its capital ($\text{\$942} \to \text{\$1.59}$) in 14 zero-swap rebalances over 65 hours. The theorem's extinction bound predicted 11 rebalances at the average leak rate; 10 donation events were observed. The three theorems together describe residual creation (Theorem~\ref{thm:residual}), equilibrium convergence (Theorem~\ref{thm:convergence}) and boundary extinction (Theorem~\ref{thm:extinction}).
...
Adams, H., Zinsmeister, N., Salem, M., Keefer, R. and Robinson, D. (2021) `Uniswap v3 Core', \emph{Uniswap Labs Technical Whitepaper}. \url{https://uniswap.org/whitepaper-v3.pdf}.

\bibitem[Baxley(2023)]{baxley2023}
Baxley, B. (2023) `Maverick Protocol: Dynamic Distribution AMM', \emph{Maverick Protocol Documentation}. \url{https://docs.mav.xyz/}.

\bibitem[Cartea et~al.(2024)]{cartea2024}
Cartea, \'A., Drissi, F. and Monga, M. (2024) `Decentralised Finance and Automated Market Making: Predictable Loss and Optimal Liquidity Provision', \emph{SIAM Journal on Financial Mathematics}, forthcoming. \url{https://doi.org/10.1137/23M1602103}. Preprint: arXiv:2309.08431.

\bibitem[Fan et~al.(2023)]{fan2023}
Fan, Z., Marmolejo-Cossío, F.J., Moroz, D., Neuder, M., Rao, R. and Parkes, D.C. (2023) `Strategic Liquidity Provision in Uniswap v3', in \emph{Proceedings of the 5th Conference on Advances in Financial Technologies (AFT 2023)}, LIPIcs vol.~282, Article 25. \url{https://doi.org/10.4230/LIPIcs.AFT.2023.25}. Preprint: arXiv:2106.12033.

\bibitem[Hasbrouck et~al.(2025)]{hasbrouck2025}
Hasbrouck, J., Rivera, T.J. and Saleh, F. (2025) `An Economic Model of a Decentralized Exchange with Concentrated Liquidity', \emph{Management Science}, forthcoming. \url{https://doi.org/10.1287/mnsc.2024.04510}. SSRN: 4529513.

\bibitem[Heimbach et~al.(2022)]{heimbach2022}
Heimbach, L., Schertenleib, E. and Wattenhofer, R. (2022) `Risks and Returns of Uniswap V3 Liquidity Providers', in \emph{Proceedings of the 4th ACM Conference on Advances in Financial Technologies (AFT 2022)}. \url{https://doi.org/10.1145/3558535.3559772}. Preprint: arXiv:2205.08904.

\bibitem[Martinelli and Mushegian(2019)]{martinelli2019}
Martinelli, F. and Mushegian, N. (2019) `A non-custodial portfolio manager, liquidity provider, and price sensor', \emph{Balancer Labs Whitepaper}. \url{https://docs.balancer.fi/whitepaper.pdf}.

\bibitem[Milionis et~al.(2022)]{milionis2022b}
Milionis, J., Moallemi, C.C., Roughgarden, T. and Zhang, A.L. (2022) `Automated Market Making and Loss-Versus-Rebalancing', \emph{arXiv preprint}, arXiv:2208.06046.

\bibitem[Milionis et~al.(2024)]{milionis2024}
Milionis, J., Moallemi, C.C. and Roughgarden, T. (2024) `Automated Market Making and Arbitrage Profits in the Presence of Fees', in \emph{Financial Cryptography and Data Security 2024}. Preprint: arXiv:2305.14604.

\bibitem[Ryan(2026b)]{ryan2026b}
Ryan, K.R.\ (2026b) `The Geometric Siphon II: Directional Properties'. \url{https://doi.org/10.2139/ssrn.6481498}.

\bibitem[Willetts and Harrington(2026)]{willetts2026}
Willetts, M. and Harrington, C. (2026) `Pools as Portfolios: Observed arbitrage efficiency \& LVR analysis of dynamic weight AMMs', \emph{arXiv preprint}, arXiv:2602.22069.
...
The repository is at \url{https://github.com/g4nc0r/the-geometric-siphon}; the Zenodo DOI \url{https://doi.org/10.5281/zenodo.19526374} resolves to an immutable archival snapshot of the repository at the time of deposit, whilst the GitHub URL tracks the latest commit. The Foundry suite reproduces all theorem verifications described in \S\ref{sec:foundry} from a clean checkout (see Appendix~\ref{app:foundry}). The diffusion-event log of 1{,}380 rebalance events across 39 positions used for the empirical analyses in \S\ref{sec:results} is available on request.
