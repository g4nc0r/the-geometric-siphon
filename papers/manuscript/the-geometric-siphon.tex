...
Concentrated liquidity, introduced in Uniswap V3,\cite{adams2021} requires active management. Every major CL automation platform (Gamma Strategies, Arrakis Finance, Beefy CLM) uses a \emph{vault-per-pool} architecture in which one vault manages one token pair and leftover tokens from rebalances are reinvested into the same position. Residuals stay in the pool that produced them and usually appear only as a small capital-efficiency loss, largely indistinguishable from slippage in standard PnL tracking.

Position Manager V6 (PMV6), a CL management contract on Aerodrome (Base), takes a different approach. Leftover tokens sit in \texttt{dustBalance[depositor][token]}, keyed by depositor and token with no pool dimension; every position under one depositor draws from the same balance. That capital-efficiency choice also opens a channel through which residuals from one pool can be consumed by positions in another.
...
When the connector token moves, every pool containing it experiences a correlated geometric mismatch on that side, whereas the non-connector token follows pool-specific dynamics. Correlated mismatches in the connector leg can therefore generate systematic net flows, while mismatches in peripheral tokens tend to average out. This is only an intuition, not a proof.
...
\textbf{Key Phase 2 design features.}
\begin{itemize}[leftmargin=1.5em,labelsep=0.5em,itemsep=2pt,topsep=4pt]
    \item \textbf{Hub-spoke topology.} WETH/USDC CL50 served as the central hub, with each spoke sharing either WETH or USDC.
    \item \textbf{$V/L_p$ sweep.} Three positions were deployed in the same pool at a $1{:}3{:}9$ size ratio, isolating position size as the only varying factor.
    \item \textbf{Fiat-FX weekend experiment.} Five stablecoin FX pairs were deployed on a Sunday, when markets were closed, under $\sigma \approx 0$ conditions that isolate the $V/L_p$ effect from volatility.
\end{itemize}
...
Among the 72 transactions involving a swap, the residual is strongly rank-correlated with observed flow: $\rho(\Delta R, D) = 0.976$ and $\rho(|\Delta R|, |D|) = 0.947$. Mean slippage is \$1.49 on a median \$102 swap, with median price impact of $0.35\%$.
...
\end{axis}
\end{tikzpicture}
\caption{$V/L_p$ transfer function. Mean $D/V$ by $V/L_p$ bucket for crypto and stablecoin FX pools. Absorption peaks near $0.05$--$0.10$, crosses zero near $0.13$, and donation intensifies above that point. Marker area scales with bucket sample size.}
...
\textbf{Recovery episodes.} The three largest absorption events (5, 8, and 12) each coincided with a substantial increase in pool liquidity $L_p$. After each one, the position returned to donation on the next rebalance, consistent with the recovery condition in the irreversibility corollary.
...
One caveat remains. The Phase~3 signal is $D = \Delta R - S$, not $\Delta R$ directly, so the directional interpretation assumes the swap-absorption fraction $S/\Delta R$ is approximately symmetric across directions; Phase~3's design does not test that assumption. Theorems~\ref{thm:directional} and~\ref{thm:exit} are stated for $V$, not $|\Delta R|$, but by Theorem~\ref{thm:residual}, $|\Delta R|/V$ depends only on range geometry and current price, so $|\Delta R|$ inherits both asymmetries.
...
\paragraph{Reversed-ordering negative control.} On reversed-ordering pairs, Theorem~\ref{thm:exit} predicts the inequality should sign-flip rather than just attenuate. P1 is the only portfolio with enough reversed-ordering exits ($N_b = 22$) for a direct test. On its reversed subset, $\bar V_b$ turns from a loss to a gain and the inequality flips as predicted (Table~\ref{tab:exit-control}).
...
The single-operator dataset of 1{,}380 events is consistent with the existence claim and includes a complete USDC/ZARP extinction lifecycle. The position falls from \$942 to \$1.59 over 14 zero-swap rebalances, against a tighter bound of 11 at the average residual fraction. The 35{,}910-event production-scale dataset, across five independent portfolios, replicates the directional asymmetry in every portfolio, the exit asymmetry in four of five, and exhibits the predicted sign flip in the largest portfolio's reversed-ordering negative control.
...
The Foundry verification suite is publicly available on GitHub at \url{https://github.com/g4nc0r/the-geometric-siphon}, with an immutable archival snapshot on Zenodo at \url{https://doi.org/10.5281/zenodo.19526374}. A clean checkout reproduces every theorem verification described in \S\ref{sec:foundry}. The diffusion-event logs supporting the empirical analyses in \S\S\ref{sec:decomposition}--\ref{sec:exit-empirical}, together with a Python reproduction harness that regenerates the empirical tables and the V3-ordering classification used for the segmented Phase~3 analysis, are deposited on Zenodo at \url{https://doi.org/10.5281/zenodo.19904832}.
...
$R(s, s_a, s_b)$ & token ratio $x/y$ demanded by range $[s_a, s_b]$ at price $s$ \\
...
\frac{\Delta R}{V} = 0 \;\iff\; R(s, s_a, s_b) = R(s, s_a', s_b').
\]
Equivalently, the rebalance has zero residual value if and only if the old and new ranges demand the same token ratio at the current price.
